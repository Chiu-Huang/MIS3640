...
while not done:
    ...
    if game_over:
        ...
        text = font.render(str(text_rect) + str(text_x), True, WHITE)
        text_rect = text.get_rect()
        text_x = screen.get_width() / 2 - text_rect.width / 2
        text_y = screen.get_height() / 2 - text_rect.height / 2
        k = [text_x, text_y,text_rect[2],text_rect[3]]
...
def drawText(>surf, text, color, rect, font, aa=False, bkg=None):
    while text:
        i = 1
        ...
        image = font.render(text[:i], False, white)
