...
let int = ("int","pair true");; 
let xor = ("xor","<a -> <b ->a (b false true) b>>");;
let helpxor = ("helpxor", "<a -> <b -> (xor (pfirst a) (pfirst b))>>")
let helptimes = ("helptimes", "<a -> <b -> times (psecond a) (psecond b)>>")

(* let neg_int = ("neg_int","if (geq _0) (pair true psecond) (pair false psecond)") ;; *)
let neg_int = ("neg_int","<k -> pair (not (pfirst k)) (psecond k)>") ;;


let plus_int1 = ("plus_int1", "<a -> <b -> pair (pfirst a) (plus (psecond a) (psecond b)) >>");;
let minus_int1 = ("minus_int1", "<a -> <b -> pair (pfirst a) (minus (psecond a) (psecond b))>>")
let minus_int2 = ("minus_int2", "<a -> <b -> pair (pfirst b) (minus (psecond b) (psecond a))>>")


let plus_int = ("plus_int", "<a -> <b -> if (helpxor a b) (if (geq (psecond a) (psecond b)) (minus_int1 a b) (minus_int2 a b)) (plus_int1 a b)>>");;

let times_int = ("times_int","<a -> <b -> if (helpxor a b) (pair false (helptimes a b)) (pair true (helptimes a b))>>")

let q1_defs : (string * string) list = 
  default_defs @ [int; neg_int; xor; plus_int1; minus_int1; minus_int2; helpxor; helptimes; plus_int; times_int];;

(* simplify q1_defs "xor (true) (true)";;
simplify q1_defs "xor (true) (false)";;
simplify q1_defs "xor (false) (true)";;
simplify q1_defs "xor (false) (false)";; *)
simplify q1_defs "geq _1 _2";;
simplify q1_defs "minus _1 _3";;
simplify q1_defs "xor pfirst (neg_int(int _3)) pfirst (neg_int(int _2))";;
simplify q1_defs "xor pfirst ((int _3)) pfirst (neg_int(int _2))";;
simplify q1_defs "xor pfirst (neg_int(int _3)) pfirst ((int _2))";;
simplify q1_defs "xor pfirst ((int _3)) pfirst ((int _2))";;
...
let alt_pred = ("alt_pred", "<n -> if (alt_iszero n) n (psecond n)>")
...
let empty = ("empty","<x -> true>")

let isempty = ("isempty", "<p -> p <x -> <y -> false>>>")

let cons = ("cons","<h -> <t -> <f -> f h t>>>")

let match_list = ("match_list","<L -> <a -> <f -> (isempty L) a (L f)>>>")

let helpsum = ("helpsum","<L -> match_list L _0 <h -> <t -> h>> >")

let sum = ("sum", "Y <sum -> <L -> (isempty L) _0 (plus (helpsum L) (sum (match_list L _0 <h -> <t -> t>>)))>>")

let map = ("map","Y <g -> <f -> <x -> isempty x empty (pair (f (pfirst x)) (g f (psecond x)))>>>")



let q3_defs : (string * string) list = 
  default_defs @ [empty; cons; isempty; match_list; helpsum; sum; map]
